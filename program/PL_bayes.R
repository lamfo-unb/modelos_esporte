...
for(i in 2){
  ...
  for(j in 1:25){
    name_file0 <- paste0("s",gsub("/","",pseason),"_pg",str_pad(j, 3, pad = "0"),".rds")
    if(file.exists(file.path(pathout,name_file0))){
      ...
      if(base_jogadores=="Not found"){
        ...
        for( k in 1:nrow(base_jogadores)){
          ...
          for(l in 1:nrow(base_v_fifa)){
            name_file <- paste0("s",gsub("/","",pseason),"_pg",
                                str_pad(j, 3, pad = "0"),"_j",
                                str_pad(k, 3, pad = "0"),"_v",
                                str_pad(l, 3, pad = "0"),".rds")
            if(file.exists(file.path(pathout,name_file))){
              print(paste0("jogador ",k," versão pulada ",l))
              next
            }else{
              cont_base <- cont_base + 1
              base_info_jogador <- raspagem3(base_v_fifa$link_atualiza[l])
              if(base_info_jogador =="sem_atualizacao"){
                next
              }else{
