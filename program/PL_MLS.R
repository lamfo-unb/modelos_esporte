...
raspagem0 <- function(link1){
  ...
  resultado <- data.table(league="MLS",link_season=paste0("https://www.fifaindex.com/",geral),league_link="/?gender=0&league=39")
}
...
link1 <- "https://www.fifaindex.com/pt-br/players/1/?gender=0&league=39"
...
for(i in 5){
  ...
  for(j in 1:25){
    name_file <- paste0("s",gsub("/","",pseason),"_pg",str_pad(j, 3, pad = "0"),"_MLS.rds")
