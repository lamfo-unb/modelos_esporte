...
raspagem0 <- function(link1){
  ...
  resultado <- data.table(league="COCA",link_season=paste0("https://www.fifaindex.com/",geral),league_link="/?gender=0&league=14&team=1939")
}
...
link1 <- "https://www.fifaindex.com/pt-br/players/1/?gender=0&league=14&team=1939"
base_seasons <- raspagem0(link1)

## seasons
for(i in 2:3){
  pseason <- gsub("https://www.fifaindex.com/pt-br/players/","",base_seasons$link_season[i])
  pseason <- ifelse(pseason=="","fifa18",pseason)
  for(j in 1:25){
    name_file <- paste0("s",gsub("/","",pseason),"_pg",str_pad(j, 3, pad = "0"),"_HUDDERS.rds")
