...
## Modelo teste, passe curto em 2015

base_result_jogadores_modelo %>%
  group_by(resultado) %>% filter(season==2015) %>%
  summarise(valor = mean(`Passe curto`))

varsmodel <- c("Passe curto")
varsmodely <- c("VA","Emp","VB")
xr  <- data.matrix(cbind(1,base_result_jogadores_modelo %>% filter(season==temporada) %>% select(varsmodel)))
yr <-  data.matrix(cbind(as.numeric(base_result_jogadores_modelo %>% filter(season==temporada) %>%
                                      select(resultado) == "VA"),
                         as.numeric(base_result_jogadores_modelo %>% filter(season==temporada) %>%
...
r1 <- maxLik(logDirichregregt,start = c(2,2,
                                        2,2,
                                        2,2),
               grad = gradients,
               method = "SANN")
...
## parametros 
matrix(r1$estimate,nrow=ncol(yr),byrow = F)
